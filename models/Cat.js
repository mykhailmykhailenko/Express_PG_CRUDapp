class Cat {
    ...
    static async getOne(catId) {
        const {rows} =  await this._client.query(`SELECT * FROM ${this._tableName} WHERE id = ${catId};`);
        return rows;
    }
    ...
    static async updateCat({catId, updateValues}) {
        const insertAttr = Object
                                ...
                                .map(([attr]) => attr);
        const insertValueString = insertAttr
                                    .map(attr => {
    ...
    static async deleteCat(catId) {
        const {rows} = await this._client.query(`DELETE FROM ${this._tableName} WHERE id = ${catId} RETURNING *;`);
        return rows;
    }
